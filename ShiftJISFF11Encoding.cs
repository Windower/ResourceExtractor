...
internal class ShiftJISFF11Encoding(ShiftJISFF11Encoding.Context context) : Encoding {
		...
		for (var i = begin; i < end; i++) {
			...
			if (b == 0xFD) {
				...
				if (c >= 0xD800 && c <= 0xDEFF) {
					written++;
					var temp = Math.Min((c >> 8) - 0xD8, end - i);
					i += temp;
					written += temp;
				} else if (c == 0xDF00) { /* skip */ } else if (c >= 0xDF01 && c <= 0xDFFF) {
					if (++i < end) {
						written++;
					}
				} else {
		...
		for (var i = begin; i < end; i++) {
			...
			if (b == 0xFD) {
				...
				if (c >= 0xD800 && c <= 0xDEFF) {
					...
					chars[charIndex + written] = (char) c;
					written++;
				}
			}
		}
	...
	[
		...
		0x0000, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD, 0xFFFD, 0x000A, 0xFFFD, 0xFFFD, 0xD90A, 0xFFFD, 0xD80C, 0xFFFD, 0xFFFD, 0xFFFD,
